...
| candidates after every screen     | 452   |
| deployed and screened so far      | 72    |
| in `rules.include`                | 46    |
| plus, per-case, in `suites`       | 3     |

Of the 72 screened (12 of which turned out to deploy nothing — see
`skipDeployment` below):

| Verdict          | Files  |
| ---------------- | ------ |
| pass             | 46 whole files, plus 6 of 8 `trailingslash`, 3 of 5 `resume-data-cache` and 3 of 7 `dynamic-route-interpolation` cases |
| fixed            | 14 defects, every one of which came from a file listed above, and all 14 verified green against a deployment |
