...
| candidates after every screen     | 465   |
| deployed and screened so far      | 51    |
| in `rules.include`                | 31    |
| plus, per-case, in `suites`       | 3     |

Of the 51 screened (11 of which turned out to deploy nothing — see
`skipDeployment` below):

| Verdict          | Files  |
| ---------------- | ------ |
| pass             | 31 whole files, plus 6 of 8 `trailingslash`, 3 of 5 `resume-data-cache` and 3 of 7 `dynamic-route-interpolation` cases |
...
| no signal        | 14 (2 gated by next.js, 12 `skipDeployment` or stubbed in deploy mode) |
