...
### `healthCheckPath` moved to the two Containers constructs

It was required on all four root constructs, but only `NextjsGlobalContainers`
and `NextjsRegionalContainers` ever read it — for the ALB target group and the
ECS container health check. The Functions types took it and ignored it: Lambda
has no long-lived process to probe, so there was nothing for it to do there.

- `NextjsGlobalFunctions` / `NextjsRegionalFunctions`: the prop no longer exists.
  **Remove it** — leaving it in place is now a type error rather than a no-op.
  The `/api/health` route in your app is only needed if something else calls it.
- `NextjsGlobalContainers` / `NextjsRegionalContainers`: unchanged, still
  required, and still prefixed with your app's `basePath` (0.6.2) before it
  reaches the target group and the container probe. Those two are now its only
  consumers — the Lambda Web Adapter readiness check it also fed in 0.6.x is gone
  along with LWA itself.
- `NextjsComputeBaseProps.healthCheckPath` moved to `NextjsContainersProps`.
