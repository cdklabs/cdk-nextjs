...
cdk-nextjs no longer sets `output: "standalone"`, and no deployment runs the
`server.js` that standalone generates. Instead, `next build` reports every route
it built through the [Deployment Adapter API](https://nextjs.org/docs/app/api-reference/adapters/runtime-integration),
cdk-nextjs stages those outputs and their traced dependencies into
...
- **If you have a custom Dockerfile**, it can no longer `COPY .next/standalone`
  or `CMD ["node", "server.js"]`. Copy `.next/cdk-nextjs-adapter/app` to the
  image root and run `node cdk-nextjs-runtime/server.mjs`. Compare against
  `src/nextjs-build/global-containers.Dockerfile`. cdk-nextjs now **rewrites**
  any Dockerfile whose first line is `# ~~ Generated by cdk-nextjs ~~`, so
  upgrades no longer leave a stale one behind; delete that line to take
  ownership of the file.
- **Monorepo detection changed mechanism.** It used to find `server.js` with a
...
### Functions are zip Lambdas, not container-image Lambdas

Both Functions types (`NextjsGlobalFunctions`, `NextjsRegionalFunctions`) now
package the staged tree as a zip asset. Docker is no longer needed to deploy
them — only the Containers types still build images.

- **`overrides.nextjsFunctions.dockerImageFunctionProps` →
  `overrides.nextjsFunctions.functionProps`.** The keys that were specific to
  container images (`assetImageCodeProps`, and anything under
...
`_next/image` is served by the same function as every other route, through
cdk-nextjs's own S3-aware optimizer, after middleware. The separate Lambda that
briefly existed behind an env var is removed along with it:

- `CDK_NEXTJS_EXPERIMENTAL_DEDICATED_IMAGE_FUNCTION` — **removed.** It was never
  a supported prop; unset it in CI and anywhere else it was exported. It is now
  ignored, not honored.
- `NextjsImageFunction` construct, `NextjsImageFunctionProps`,
  `NextjsImageFunctionOverrides`,
  `NextjsGlobalFunctions.nextjsImageFunction`,
  `NextjsRegionalFunctions.nextjsImageFunction` — removed.
...
Independent memory and timeout sizing for image requests is the cost. Raise
`overrides.nextjsFunctions.functionProps.memorySize` if image latency regresses.

### New (non-breaking): `functionGroups`

Splits one Next.js app across several Lambda functions, one per declared group of
routes, so an app too large for Lambda's 250 MB unzipped limit can still deploy.
